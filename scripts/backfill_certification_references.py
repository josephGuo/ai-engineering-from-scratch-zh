...
def assessment_paths(track: dict) -> list[Path]:
    paths = []
    assessment_root = (ROOT / "certifications" / "claude" / "assessments").resolve()
    for declaration in track.get("assessments", []):
        relative = declaration.get("path")
        if isinstance(relative, str):
            candidate = (ROOT / relative).resolve()
            if not candidate.is_relative_to(assessment_root):
                raise ValueError(f"assessment path escapes certifications/claude/assessments: {relative}")
