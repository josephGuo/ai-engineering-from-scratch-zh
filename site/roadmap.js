(function () {
  ...
  var STAGES = [
    ...
    { id: 'model-disciplines', number: '02', name: '模型方向', startTier: 4, endTier: 6, focusPhase: 7 },
    { id: 'engineering-systems', number: '03', name: '工程系统', startTier: 7, endTier: 11, focusPhase: 11 },
    { id: 'capstone-proof', number: '04', name: '综合验证', startTier: 12, endTier: 12, focusPhase: 19 }
  ];
  ...
  function renderStageNavigation() {
    ...
    for (var i = 0; i < STAGES.length; i++) {
      ...
      html += '<button class="roadmap-stage-jump" type="button" data-stage-target="' + stage.id + '">' +
        '<span>分区 ' + stage.number + '</span><strong>' + escapeHtml(stage.name) + '</strong>' +
      '</button>';
    }
    nav.innerHTML = html;
  }
  ...
  function renderJumpOptions() {
    ...
    if (!select) return;
    var html = '<option value="">跳到某个阶段</option>';
    for (var i = 0; i < PHASES.length; i++) {
      var phase = PHASES[i];
  ...
  function renderHeroStats() {
    ...
    setText('roadmapNextPhase', recommendation ? '阶段 ' + formatPhase(recommendation.id) : '全部完成');
  }
  ...
  function renderStageBands(layer) {
    for (var i = 0; i < STAGES.length; i++) {
      ...
      number.textContent = '分区 ' + stage.number;
  ...
  function phaseNarration(phase, state, progress) {
    ...
    var text = '阶段 ' + formatPhase(phase.id) + '：' + phase.name + '。' + state.label + '。' +
      '已完成 ' + progress.done + ' / ' + progress.total + ' 节课。';
    text += requirements.length ? '直接前置阶段：' + requirements.join('、') + '。' : '这是起点。';
    text += unlocks.length ? '直接解锁：' + unlocks.join('、') + '。' : '这是最终阶段。';
    return text;
  }
  ...
  function clearSelection(updateHistory) {
    ...
    if (jump) jump.value = '';
    renderEmptyInspector();
    setText('roadmapGraphStatus', '已清除路线聚焦。');
  ...
  function restoreSelectionFromUrl(shouldCenter) {
    ...
    if (!phaseMap[id]) {
      if (selectedId !== null) clearSelection(false);
      setText('roadmapGraphStatus', '链接没有匹配到路线图阶段。');
      return;
    }
  ...
  function renderEmptyInspector() {
    ...
    var recommendationHtml = recommendation
      ? '<div class="roadmap-recommendation"><span>建议下一步</span><button type="button" data-route-phase="' + recommendation.id + '">阶段 ' + formatPhase(recommendation.id) + ' · ' + escapeHtml(recommendation.name) + '</button></div>'
      : '';
    panel.innerHTML =
      '<span class="roadmap-inspector-eyebrow">路线检查器</span>' +
      '<h2>选择一个阶段</h2>' +
      '<p class="roadmap-inspector-copy">选择节点即可查看通往它的完整路线、它会解锁的所有阶段，以及最适合从本地进度继续的课程。</p>' +
      recommendationHtml;
  }
  ...
  function renderInspector(id) {
    ...
    var actionLabel = progress.done === progress.total && progress.total > 0 ? '复习本阶段' : (progress.done > 0 ? '继续本阶段' : '开始本阶段');
    panel.innerHTML =
      ...
      '<div class="roadmap-inspector-context">' +
        '<div class="roadmap-inspector-stat"><strong>' + Object.keys(ancestors).length + '</strong><span class="roadmap-inspector-stat-label">全部前置阶段</span></div>' +
      ...
      '<div class="roadmap-route-sections">' +
        renderRouteSection('直接前置阶段', directPrereqs, '这是起点。') +
        renderRouteSection('直接解锁内容', directUnlocks, '这是最终阶段。') +
      '</div>' +
      '<div class="roadmap-actions">' +
        (lessonLink ? '<a class="roadmap-action roadmap-action-primary" href="' + lessonLink + '">' + actionLabel + '</a>' : '') +
        '<a class="roadmap-action" href="' + phaseGithubUrl(phase) + '" target="_blank" rel="noopener">在 GitHub 查看本阶段</a>' +
      '</div>';
  }
  ...
  function announceSelection(id) {
    ...
    setText('roadmapGraphStatus', '已选择阶段 ' + formatPhase(id) + '。已高亮 ' + ancestors + ' 个前置阶段和 ' + descendants + ' 个后续阶段。');
  }
  ...
  function lessonPageUrl(lesson) {
    ...
    return path ? '/lessons/' + path.replace(/^phases\//, '') + '/' : '';
  }
  ...
  function phaseGithubUrl(phase) {
    return 'https://github.com/fancyboi999/ai-engineering-from-scratch-zh/tree/main/phases/' + extractPhaseSlug(phase);
  }
