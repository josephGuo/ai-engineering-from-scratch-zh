"""配套代码：
certifications/claude/lessons/12-claude-agent-sdk-and-hooks/docs/zh.md
用于校验 tool、hook、沙箱、预算、subagent 隔离与恢复状态。
本策略实验无需安装 Agent SDK，也无需服务商凭证。
"""
...
class EventStreamError(ValueError):
    """managed agent 事件 fixture 违反状态合约时抛出。"""
...
def consume_managed_events(events: Iterable[dict[str, Any]]) -> EventSummary:
    """在不访问网络的情况下消费持久化事件与可选 stream 预览。"""
    status = "unknown"
