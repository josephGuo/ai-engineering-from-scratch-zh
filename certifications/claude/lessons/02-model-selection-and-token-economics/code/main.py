"""配套代码：
certifications/claude/lessons/02-model-selection-and-token-economics/docs/zh.md
用于验证并汇总包含十个案例的本地模型路由基准测试。
还会依据带日期的支持证据与门槛，验证重复模式试验。
费率采用说明性单位，因此不需要供应商价格或凭证。
"""
